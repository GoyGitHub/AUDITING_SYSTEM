...
// --- Handle approve/disapprove (update status then redirect to coaching) ---
if (isset($_GET['approve']) || isset($_GET['disapprove'])) {
    $actionId = intval($_GET['approve'] ?? $_GET['disapprove']);

    // fetch agent name for this comment
    $agentRes = $conn->query("SELECT agent_name FROM supervisor_comments WHERE id = $actionId LIMIT 1");
    $agentName = '';
    if ($agentRes && $ar = $agentRes->fetch_assoc()) {
        $agentName = trim($ar['agent_name']);
    }

    if (isset($_GET['approve'])) {
        $conn->query("UPDATE supervisor_comments SET status='approved' WHERE id=$actionId");

        // If agent name available, ensure it exists in agents2 (split to firstname/lastname)
        if (!empty($agentName)) {
            // conservative split: first token => firstname, rest => lastname
            $parts = preg_split('/\s+/', $agentName, 2);
            $first = $parts[0] ?? $agentName;
            $last = $parts[1] ?? '';

            // Check if exists
            $check = $conn->prepare("SELECT id FROM agents2 WHERE agent_firstname = ? AND agent_lastname = ? LIMIT 1");
            $check->bind_param("ss", $first, $last);
            $check->execute();
            ...
            if (!$exists) {
                // Insert minimal record; optional fields left blank
                $insert = $conn->prepare("INSERT INTO agents2 (agent_firstname, agent_lastname) VALUES (?, ?)");
                $insert->bind_param("ss", $first, $last);
                $insert->execute();
                $insert->close();
            }
        }
        ...
        $conn->query("UPDATE supervisor_comments SET status='disapproved' WHERE id=$actionId");
    }

    // Redirect to Conduct Coach page with agent name (if available)
    $redirect = "SupervisorConductCoach.php";
    if (!empty($agentName)) {
        $redirect .= "?agent=" . urlencode($agentName);
    }
    header("Location: " . $redirect);
    exit();
}

// --- Handle supervisor comment submission (manual add from supervisor) ---
            ...
            <tbody>
                <?php if (count($comments)): ?>
                    <?php foreach ($comments as $c):
                        ...
                        $filedByRole = '';
                        $filedByName = '';
                        // Try to match "(Filed by: ROLE - NAME)"
                        if (preg_match('/\s*\(Filed by:\s*([^-\)]+)\s*-\s*([^)]+)\)\s*$/i', $rawComment, $m)) {
                            $filedByRole = trim($m[1]);
                            $filedByName = trim($m[2]);
                            $displayComment = preg_replace('/\s*\(Filed by:\s*([^-\)]+)\s*-\s*([^)]+)\)\s*$/i', '', $rawComment);
                        } elseif (preg_match('/\s*\(Filed by:\s*([^)]+)\)\s*$/i', $rawComment, $m2)) {
                            // fallback to previous single value
                            $filedByName = trim($m2[1]);
                            $displayComment = preg_replace('/\s*\(Filed by:\s*([^)]+)\)\s*$/i', '', $rawComment);
                        } else {
                    ...
                    ?>
                        <tr>
                            ...
                            <td><?php echo nl2br(htmlspecialchars($displayComment)); ?></td>
                            <td>
                                <?php
                                    if ($filedByRole && $filedByName) {
                                        echo htmlspecialchars($filedByRole . ': ' . $filedByName);
                                    } elseif ($filedByName) {
                                        echo htmlspecialchars($filedByName);
                                    } else {
                                        echo '<em>Unknown</em>';
                                    }
                                ?>
                ...
                <?php else: ?>
                    <tr>
                        <td colspan="6" style="text-align:center;">No pending supervisor comments.</td>
                    </tr>
                <?php endif; ?>
            </tbody>
        </table>
